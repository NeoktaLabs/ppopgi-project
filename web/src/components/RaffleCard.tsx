...
interface RaffleCardProps {
  ...
  ticketPrice: string;
  sold: number;
  minTickets: number; // ADDED: Needed for "Goal" logic
  maxTickets: number; // ADDED: Needed for "Cap" logic
  endsIn: string;
  color: any;
  creator?: string;
  rank?: number;
...
export function RaffleCard({ address, deployer, title, prize, ticketPrice, sold, minTickets, maxTickets, endsIn, color, creator, rank }: RaffleCardProps) {
  ...
  // LOGIC FIX: Only calculate percent if maxTickets exists
          ...
          <div className="flex-1 p-4 flex flex-col justify-center space-y-3">
             {hasHardCap ? (
               ...
               <>
                 ...
                 <div className={`flex items-center gap-2 p-2 rounded-lg text-xs font-bold ${minReached ? 'bg-green-50 text-green-600' : 'bg-gray-100 text-gray-500'}`}>
                   <Target size={14} /> {minReached ? "Minimum Met (Active)" : "Funding..."}
                 </div>
               </>
             )}
